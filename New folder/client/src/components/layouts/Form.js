...
class Form extends Component{
  ...
  async handleFormSubmit( event ) {
    ...
    const auth_token = this.props.auth_token;
    console.log(auth_token);
    const body = JSON.stringify({ name,fatherName,mobile,email,applicantType,district,areaType,address1,address2,pincode,road_district,road_urban_local_body_name,road_ward_no,road_locality,road_cuttingReason,road_category,road_totalcost,auth_token});
    console.log(body);
    const config = {
      headers: {
  ...
  <input type="text" id="road_totalcost" name="road_totalcost" placeholder="in Rs."
    ...
    <br></br>
    <input type="submit" onClick={e => this.handleFormSubmit(e)} value="Submit and pay later" />
    <input type="submit" onClick={e => this.handleFormSubmit(e)} value="Submit and pay now" />
</form >
    </div>
    </div>
    );
